...
const OrganizerDashboard: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        'Are you sure you want to delete this event? This will permanently cancel all user registrations.'
      )
    ) {
      return;
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-panel p-5 rounded-xl border border-white/5 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-white mt-1">{stats?.myEventsCount || 0}</p>
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-gradient-to-r from-primary to-secondary hover:from-primary-dark hover:to-secondary-dark text-white font-semibold text-xs px-4 py-2.5 rounded-lg shadow-lg hover:shadow-primary/20 transition-all duration-200"
          >
        ...
        {events.length === 0 ? (
          <div className="glass-panel text-center py-16 rounded-xl border border-white/5">
            <span className="text-4xl">🎟️</span>
            <h4 className="text-md font-bold text-white mt-4">Create your first event</h4>
            <p className="text-xs text-gray-400 mt-2 max-w-xs mx-auto">
              Get started by clicking the "Create Event" button above. Add details, set capacity
              limit, and upload a poster.
            </p>
          </div>
        ) : (
      ...
      {/* Form Modal overlay */}
      {isFormOpen && (
        <div className="fixed inset-0 z-50 bg-[#090d16]/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="glass-panel rounded-2xl border border-white/10 w-full max-w-4xl p-6 md:p-8 max-h-[90vh] overflow-y-auto relative shadow-2xl">
            <button
